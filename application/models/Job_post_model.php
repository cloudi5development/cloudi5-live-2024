...
class Job_post_model extends MY_Model{
	...
	public $_table_name 	= 'job_post';
    public $_order_by 		= 'job_post_id';
    public $_primary_key	= 'job_post_id';
	...
	public function getBySlugId($job_id,$slug)
  	{ 
	   return $this->db->where([
  			'job_id' => strtoupper($job_id),'slug' => $slug,'status' =>'Active'
  		])->get($this->_table_name)->row();
	}
	...
	var $column_order = array(Null,'job_post_date','job_id','title','no_of_vacancy','status','status'); 
	var $column_search = array('job_post_date','job_id','title','no_of_vacancy','status');
	var $order = array('job_post_id' => 'DESC'); 
